...
_OG_IMAGE_W, _OG_IMAGE_H = 1200, 630     # keep in sync with static/og.png's actual dimensions
_OG_IMAGE_ALT = ("Garmin — Claude & ChatGPT answer from your own Garmin "
                 "data.")
...
def render_page(fragment: str, title: str, desc: str | None = None, *,
                ...
                extra_head: str = "", social_desc: str | None = None,
                bare: bool = False) -> str:
    ...
    the search description is too long to survive a preview intact. `bare`
    strips the marketing header nav and footer (credential forms shouldn't
    carry donation links or anchor-links to sections of a page they're not
    on) while keeping the shared <style> and <head> — see oauth.py's
    authorize/second-factor pages."""
    desc = desc or _DEFAULT_DESC
    out = (tpl("_layout.html")
           .replace("{TITLE}", title)
           .replace("{DESC}", desc)
    ...
    if bare:
        out = re.sub(r"<header>.*?</header>\s*", "", out, flags=re.DOTALL)
        out = re.sub(r"<footer>.*?</footer>\s*", "", out, flags=re.DOTALL)
    return out
