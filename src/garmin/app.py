...
def build_app(config: Config) -> Starlette:
    ...
    home_page = _render(
        "home.html", "Garmin — Your Garmin data, in Claude & ChatGPT · Garmin MCP Server",
        ...
        extra_head=_json_ld({"@type": "WebSite", "name": "Garmin",
    ...
    garmin_chatgpt_page = _render(
        "garmin_chatgpt.html",
        "Connect Garmin to ChatGPT — Garmin MCP Server | Garmin",
    ...
    privacy_page = _render(
        "privacy.html", "Privacy — Garmin",
        "What Garmin stores, what it never stores, and how to revoke "
    ...
    async def notfound(request):
        # Catch-all for unknown GET paths: humans get the Garmin home
    ...
    # Brand assets (the Garmin logo mark). Read once at startup and served
    ...
    def _server_card(a):
        return json.dumps({
            ...
            "serverInfo": {"name": f"garmin-{a.name}",
                           "title": f"{a.display_name} — Garmin",
                           "version": "1.0.0"},
    ...
    def adapter_routes(adapter):
        ...
        # people actually search for (for example, "garmin mcp").
        ...
        landing_page = _render(
            ...
            extra_head=_json_ld({
                ...
                "name": f"{adapter.display_name} MCP Server (Garmin)",
