...
def _authorize_page(adapter, config) -> str:
    ...
    # bare: a credential form has no business carrying the marketing nav
    # (donation link, anchors into homepage sections it isn't on) or the
    # site footer.
    ...
    return pages.render_page(
        adapter.authorize_template, f"Connect {adapter.display_name} — Garmin",
        noindex=True, extra_head=telemetry.web_head(config), bare=True,
    ).replace("{OPERATOR}", pages.operator_html(config))


def _second_factor_page(adapter, config) -> str:
    return pages.render_page(
        adapter.second_factor_template, f"{adapter.display_name} verification — Garmin",
        noindex=True, extra_head=telemetry.web_head(config), bare=True,
    ).replace("{OPERATOR}", pages.operator_html(config))
...
def _upstream_error(config, adapter, message: str) -> HTMLResponse:
    ...
    body = _fill(pages.render_page("upstream_error.html",
                                   f"Connect {adapter.display_name} — Garmin",
                                   noindex=True),
...
async def authorize_callback(request, adapter, state, conn, config) -> HTMLResponse | RedirectResponse:
    ...
    try:
        ...
        # verify is a blocking network step — bound it off the loop exactly
        # like the login/MFA paths, so a rate-limiting provider can't freeze
        # the single-node event loop.
        t0 = time.monotonic()
        name = await _bounded(config, adapter.verify, result.blob)
