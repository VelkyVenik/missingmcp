...
- **`app.py`** — `build_app(config)` wires routes + security-headers middleware + shared singletons (db conn, one `WorkerManager` **per worker-based adapter** — remote and local adapters get none, `AuthState`, `RateLimiter`), a per-adapter landing route rendered from `adapter.landing_template`, and a lifespan that periodically reaps idle workers (all managers) and cleans expired codes. `main()` is the `missingmcp` console entrypoint.
